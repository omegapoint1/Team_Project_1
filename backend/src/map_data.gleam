...
pub fn generate_map_data(db: pog.Connection) -> Int {
  ...
  let _ = list.map(init_data, fn(map_data) {
    let assert Ok(map_data_local) = json.parse(map_data, map_json.map_data_item_decoder())
    sql.map_data_insert(db, map_data_local.lat, map_data_local.long, map_data_local.noise, map_data_local.time, map_data_local.category)
  })
